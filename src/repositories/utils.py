...
def get_available_rooms(date_from: date, date_to: date, hotel_id: int | None = None):
    booked_rooms_count = (
    ...
    a_rooms_table = (
        select(
            ...
            (RoomsOrm.quantity - func.coalesce(booked_rooms_count.c.rooms_booked, 0)).label(
                "available_rooms"
            ),
        )
    ...
    rooms_ids_for_get = rooms_ids_for_get.select_from(a_rooms_table).filter(
